...
// template<typename T>
// bool checkLeftContext(ParseTreeNode<LeftSideNodeContent, T> * contextNode, ParseTreeNode<InstanceNodeContent, T> * instanceNode, std::vector<T> * ignore, Scope * paramMapping);
// template<typename T>
// bool checkRightContext(ParseTreeNode<LeftSideNodeContent, T> * contextNode, ParseTreeNode<InstanceNodeContent, T> * instanceNode, std::vector<T> * ignore, Scope * paramMapping);

// template<typename T>
// ParseTreeNode<InstanceNodeContent, T> * evaluateRightNode(const ParseTreeNode<RightSideNodeContent, T> * node, Scope * paramMapping);

// template<typename T>
// ParseTreeNode<InstanceNodeContent, T> * derive(ParseTreeNode<InstanceNodeContent, T> * node, Table<T> * table, std::vector<T> * ignore=nullptr, Scope * scope=nullptr);

// Scope * mapArgs(const std::vector<Value> * values, const std::vector<Parameter *> * params, Scope * paramMapping);

// bool checkCondition(LSysDParser::ExpressionContext * cond, Scope * paramMapping);

// template<typename T>
// Rule<T> * chooseRule(std::vector<Rule<T> *> * rules);
...
// extern template ParseTreeNode<InstanceNodeContent, char> * derive(ParseTreeNode<InstanceNodeContent, char> * node, Table<char> * table, std::vector<char> * ignore, Scope * env);
