...
class Derivator {
    ...
    bool checkSideContext(
            TreeNode<LeftSideNodeContent, T> * contextNode, 
            TreeNode<InstanceNodeContent, T> * instanceNode, 
            bool rightSide, std::vector<T> const* ignore, 
            Scope * paramMapping);
    bool checkLeftContext(
            TreeNode<LeftSideNodeContent, T> * contextNode, 
            TreeNode<InstanceNodeContent, T> * instanceNode, 
            std::vector<T> const* ignore, 
            Scope * paramMapping);
    bool checkRightContext(
            TreeNode<LeftSideNodeContent, T> * contextNode, 
            TreeNode<InstanceNodeContent, T> * instanceNode, 
            std::vector<T> const* ignore, 
            Scope * paramMapping);
    TreeNode<InstanceNodeContent, T> * evaluateRightNode(TreeNode<RightSideNodeContent, T> const* node, Scope * paramMapping);
    bool checkArgs(std::vector<Value> const* values, std::list<Parameter *> const* params, Scope * paramMapping);
    void mapArgs(std::vector<Value> const* values, std::list<Parameter *> const* params, Scope * paramMapping);
...
public:
    ...
    TreeNode<InstanceNodeContent, T> * derive(
            TreeNode<InstanceNodeContent, T> const* node, 
            Table<T> const* table, 
            std::vector<T> const* ignore=nullptr, 
            Scope * scope=nullptr);
