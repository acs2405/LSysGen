...
std::string node2svg(
    ...
    svg += "<!-- ";
    if (lsystem != nullptr)
        svg += "L SYSTEM " + lsystem->name() + " ITERATION " + std::to_string(lsystem->iteration()) + " - ";
    svg += "AUTOMATICALLY GENERATED BY LSYSGEN " + std::string(LSYSGEN_VERSION);
    if (lsystem != nullptr)
        svg += " WITH SEED " + std::to_string(lsystem->seed());
    svg += " -->\n";
...
std::string node2svg(
    ...
    for (node = parent->leftmostChild();
        ...
        if (node->isLeaf()) {
            ...
            if (node->element() == 'P') {
                if (values == nullptr || values->size() == 0 || values->size() == 3 || values->size() == 4) {
                    if (fill) {
                        // TODO error
                    } else {
